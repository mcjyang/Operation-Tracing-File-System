 ...
 *
 * trfs.h
 * - add a super_magic number
 * - add new fields of sb_info
 * - extern new functions: trfs_record
 * - modify trfs_set_lower_super
 */
...
/* trfs file systems superblock magic */
#define TRFS_SUPER_MAGIC 0xf15f
...
/* new stuff */
extern int trfs_record(struct super_block *sb, char* pathname, unsigned char op_type, 
				int rval);
...
/* lower device name and option */
/* new stuff */
struct trfs_dev_w_opt{
...
static inline void trfs_set_lower_super(struct super_block *sb,
					  struct super_block *val, char* out_file, 
					  unsigned int record_id, unsigned int bitmap)
{
	TRFS_SB(sb)->lower_sb = val;
	TRFS_SB(sb)->cur_id = record_id;
	TRFS_SB(sb)->op_name = out_file;
