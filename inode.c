 ...
 *
 * inode.c:
 * - Modify trfs_create for recording.
 * - Modify trfs_unlink for recording.
 */
...
out:
	...
	path_name = dentry_path_raw(lower_dentry, path_buf, 4096);
	
	bytes = trfs_record(dir->i_sb, path_name, OP_CREATE, err);
	printk("record done at create, bytes: %d \n", bytes);
...
out:
	...
	bytes = trfs_record(dir->i_sb, path_name, OP_CREATE, err);
	printk("record done at remove, bytes: %d \n", bytes);
